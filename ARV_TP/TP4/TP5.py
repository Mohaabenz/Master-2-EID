...
# -*- coding: utf-8 -*-
"""
Created on Mon Oct 21 10:21:19 2019
...
def transforme_2D_QNLdirect(A, Lmin):
    N = len(A)
    work = np.zeros((N,N))
...
def transforme_2D_QNLinverse(A, Lmin):
    N = len(A)
    n = (2**Lmin)*2
    work = np.copy(A)
    Lmax = int(math.log2(N))
...
print('QNLDirect(Image)\n')
t1 = transforme_2D_QNLdirect(image2,6)
print(t1,'\n')
plt.imshow(t1, interpolation='nearest')
plt.show()


print('QNLInverse(Image)\n')
t2 = transforme_2D_QNLinverse(t1,6)
print(t2)
...
print('BRUITAAAAAAAA')
bruit = bruitage(image2, 0.05)
bruit = np.array(bruit,float)
print(bruit, '\n')
plt.imshow(bruit)
plt.show()

bruit2 = transforme_2D_QNLdirect(bruit,6)
print(bruit2,'\n')
plt.imshow(bruit2)
plt.show()
...
t3 = seuillage(bruit2,0.05)
print('SEUILLAGE\n',t3)
plt.imshow(t3)
plt.show()


t4 = transforme_2D_QNLinverse(t3,6)
print(t4)
plt.imshow(t4)
plt.show()
